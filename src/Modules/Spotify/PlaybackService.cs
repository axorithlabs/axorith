...
internal sealed class PlaybackService : IDisposable
{
    ...
    private readonly Lock _refreshLoopLock = new();
    ...
    {
        if (!_settings.EnablePlayback.GetCurrentValue())
        {
            _logger.LogInfo("Playback control is disabled. Skipping playback.");
            return;
        }
    ...
    private void RebuildPlaybackChoices()
    {
        ...
        foreach (var playlist in _cachedPlaylists.Where(playlist => !string.IsNullOrWhiteSpace(playlist.Key)))
        {
